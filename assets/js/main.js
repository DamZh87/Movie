...
fetch(
   `https://api.themoviedb.org/3/movie/${randomFilmId}?language=ru-RU&sort_by=popularity.asc`,
   options
)
   ...
   .then((randomFilmRes) => {
      ...
      document.querySelector(".random__hero-name").textContent =
         randomFilm.title;
      document.querySelector(".random__descript").textContent =
         randomFilm.overview;
      document.querySelector(".random__year").textContent =
         randomFilm.release_date.slice(0, 4);
      document.querySelector(".random__year").textContent =
         randomFilm.release_date.slice(0, 4);
      document.querySelector(".random__movie-rating").textContent =
         randomFilm.vote_average.toString().slice(0, 3);
      document
         .querySelector(".random__hero-name")
         .setAttribute("data-rand-id", randomFilm.id);

      document
         .querySelector(".random__hero-name")
         .addEventListener("click", () => {
            const filmId = document
               .querySelector(".random__hero-name")
               .getAttribute("data-rand-id");
            location.href = "film.html";
            localStorage.setItem("filmId", filmId);
         });

      document
         .querySelector(".random_film_trailer_btn")
         .addEventListener("click", () => {
            fetch(
               ` https://api.themoviedb.org/3/movie/${randomFilmId}/videos?language=ru-RU`,
               options
            ) // Трейлер
...
fetch(
   `https://api.themoviedb.org/3/movie/${randomFilm1Id}?language=ru-RU&sort_by=popularity.asc`,
   options
)
   .then((randomFilmRes1) => randomFilmRes1.json())
   .then((randomFilmRes1) => {
      let randomFilm1 = randomFilmRes1;
      console.log(randomFilm1);
      ...
      document
         .querySelector(".random__movie-name_1")
         .setAttribute("data-rand-id", randomFilm1.id);
      ...
      document
         ...
         fetch(
            `https://api.themoviedb.org/3/movie/${randomFilm2Id}?language=ru-RU&sort_by=popularity.asc`,
            options
         )
            .then((randomFilmRes2) => randomFilmRes2.json())
            .then((randomFilmRes2) => {
               let randomFilm2 = randomFilmRes2;
               console.log(randomFilm2);
               ...
               document
                  .querySelector(".random__movie-name_2")
                  .setAttribute("data-rand-id", randomFilm2.id);
               ...
               fetch(
                  `https://api.themoviedb.org/3/movie/${randomFilm3Id}?language=ru-RU&sort_by=popularity.asc`,
                  options
               )
                  .then((randomFilmRes3) => randomFilmRes3.json())
                  .then((randomFilmRes3) => {
                     let randomFilm3 = randomFilmRes3;
                     console.log(randomFilm3);
                     ...
                     document
                        .querySelector(".random__movie-name_3")
                        .setAttribute("data-rand-id", randomFilm3.id);
...
fetch(
   `https://api.themoviedb.org/3/movie/popular?include_adult=false&language=ru-RU&page=1`,
   options
)
...
function renderMovieswiperSlide({
   ...
   headerHeroButtonsWatchswiperHeaderEl.className =
   ...
   headerHeroButtonsContinueEl.addEventListener("click", () => {
      ...
      fetch(
         ...
         .then((trailerID) => {
            ...
            document
               .querySelector(".trailer")
               .setAttribute("src", `https://www.youtube.com/embed/${trailer}`);
         })
         .catch((err) => console.error(err));
   });
...
fetch(
   `https://api.themoviedb.org/3/movie/top_rated?language=ru-RU&page=${pageTop}`,
   options
)
...
function renderSearch({ title = "", id = " " }) {
...
function updateValue(e) {
   ...
   fetch(
      `https://api.themoviedb.org/3/search/movie?include_adult=false&language=ru-RU&query=${find}`,
      options
   )
